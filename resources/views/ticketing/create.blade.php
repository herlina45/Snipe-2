...
<form action="{{ $formAction }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="date" name="requested_date" id="requested_date" class="form-control"
            value="{{ old('requested_date', isset($item->requested_date) ? \Carbon\Carbon::parse($item->requested_date)->format('Y-m-d\TH:i') : '') }}" required>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="date" name="required_date" id="required_date" class="form-control"
            value="{{ old('required_date', isset($item->required_date) ? \Carbon\Carbon::parse($item->required_date)->format('Y-m-d\TH:i') : '') }}">
    </div>
    ...
    <div class="form-group">
        ...
        <select name="status" id="status" class="form-control" required>
            ...
            @foreach($statuses as $status)
                <option value="{{ $status }}" {{ old('status', $item->status ?? 'Waiting for approval') == $status ? 'selected' : '' }}>
                    {{ $status }}
                </option>
            @endforeach
        </select>
